...
void Cde_ouverture_porte(){
 ...
 Cde_lum_auto= digitalRead (Bp_lumiere_auto);
  ...
  delay (3000);
   ValVitesse=255;
   Courant_sup=3.0;
 for (int i = 0; i <= compteur_vitesse; i++){
...
void Calcul_Courant(){
for ( x = 0; x <= compteur_courant; x++){ //boucle de  80 Echanillons
  adc = analogRead (A0);;     //lecture du courant sue A0
  ...
  if ((Acs_712 >Courant_sup)) {
    analogWrite(LPWM,0);
    analogWrite(RPWM,0);
  Serial.print( "depassement_courant  ");
  Serial.println( Acs_712);
